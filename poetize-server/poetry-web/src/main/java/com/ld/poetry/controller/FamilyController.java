...
public class FamilyController {
    ...
    public PoetryResult saveFamily(@Validated @RequestBody FamilyVO familyVO) {
        ...
        family.setBgCover(familyVO.getBgCover());
        family.setManCover(familyVO.getManCover());
        family.setWomanCover(familyVO.getWomanCover());
        family.setManName(familyVO.getManName());
        family.setWomanName(familyVO.getWomanName());
        family.setTiming(familyVO.getTiming());
        family.setCountdownTitle(familyVO.getCountdownTitle());
        family.setCountdownTime(familyVO.getCountdownTime());
        family.setFamilyInfo(familyVO.getFamilyInfo());
        ...
        if (oldFamily != null) {
            ...
            persisted = familyService.updateById(family);
        } else {
            family.setId(null);
            persisted = familyService.save(family);
        }
        if (!persisted) {
            return PoetryResult.fail("表白墙信息保存失败！");
        }
        if (userId.intValue() == PoetryUtil.getAdminUser().getId().intValue()) {
    ...
    public PoetryResult<List<FamilyVO>> listRandomFamily(@RequestParam(value = "size", defaultValue = "10") Integer size) {
        if (size == null || size < 1 || size > 100) {
            return PoetryResult.fail("size 必须在 1 到 100 之间！");
        }
        List<FamilyVO> cached = commonQuery.getFamilyList();
        List<FamilyVO> familyList = cached == null ? new ArrayList<>() : new ArrayList<>(cached);
        if (familyList.size() > size) {
            Collections.shuffle(familyList);
            familyList = new ArrayList<>(familyList.subList(0, size));
        }
