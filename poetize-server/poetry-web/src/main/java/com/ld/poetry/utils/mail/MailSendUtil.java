...
public class MailSendUtil {
    ...
    public void sendCommentMail(CommentVO commentVO, Article one, CommentService commentService) {
        ...
        if (!CollectionUtils.isEmpty(mail)) {
            ...
            int count = PoetryCache.increment(
                    CommonConst.COMMENT_IM_MAIL + mail.get(0), CommonConst.CODE_EXPIRE);
            if (count <= CommonConst.COMMENT_IM_MAIL_COUNT) {
    ...
    private String getCommentMail(String commentType, String source, String fromName, String fromContent, String toName, Integer toCommentId, CommentService commentService) {
        WebInfo webInfo = (WebInfo) PoetryCache.get(CommonConst.WEB_INFO);
        String webName = escape(webInfo == null ? "POETIZE" : webInfo.getWebName());
        String safeSource = escape(source);
        String safeFromName = escape(fromName);
        String safeFromContent = escape(fromContent);
        String safeToName = escape(toName);

        String mailType = "";
        String toMail = "";
        if (StringUtils.hasText(toName)) {
            mailType = String.format(MailUtil.replyMail, safeFromName);
            Comment toComment = commentService.lambdaQuery().select(Comment::getCommentContent).eq(Comment::getId, toCommentId).one();
            if (toComment != null) {
                toMail = String.format(MailUtil.originalText, safeToName, escape(toComment.getCommentContent()));
            }
        } else {
            if (CommentTypeEnum.COMMENT_TYPE_MESSAGE.getCode().equals(commentType)) {
                mailType = String.format(MailUtil.messageMail, safeFromName);
            } else if (CommentTypeEnum.COMMENT_TYPE_ARTICLE.getCode().equals(commentType)) {
                mailType = String.format(MailUtil.commentMail, safeSource, safeFromName);
            } else if (CommentTypeEnum.COMMENT_TYPE_LOVE.getCode().equals(commentType)) {
                mailType = String.format(MailUtil.loveMail, safeFromName);
            }
        }
    ...
    private String getImMail(String fromName, String fromContent) {
        ...
        return String.format(mailUtil.getMailText(),
                webName,
                String.format(MailUtil.imMail, safeFromName),
                safeFromName,
                escape(fromContent),
                "",
                webName);
