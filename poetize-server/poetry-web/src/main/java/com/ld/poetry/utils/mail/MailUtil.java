...
public class MailUtil {
    ...
    public void sendMailMessage(List<String> to, String subject, String text) {
        ...
        try {
            ...
            // 群发使用密送，避免订阅者邮箱互相暴露。
            if (to.size() == 1) {
                mimeMessageHelper.setTo(to.get(0));
            } else {
                mimeMessageHelper.setTo(sendMailer);
                mimeMessageHelper.setBcc(to.toArray(new String[0]));
            }
            //邮件主题
            ...
            log.info("邮件发送成功，收件人数：{}", to.size());
        } catch (MessagingException e) {
            log.error("邮件发送失败，收件人数：{}", to.size(), e);
        }
    }
}
