    ...
    <div class="friend-box" v-if="!$common.isEmpty($store.state.currentUser)">
      ...
      <div class="friend-bode" id="body-group">
        <div class="body-left" v-show="showBodyLeft">
          ...
          <div class="aside-list" v-show="type === 1">
            ...
            <div style="overflow-y: scroll;height: calc(100% - 70px)">
              ...
              <div class="im-user im-user-current"
                   v-for="item in imChats"
                   :key="'friend-chat-' + item"
          ...
          <div class="aside-list" v-show="type === 2">
            ...
            <div style="overflow-y: scroll;height: calc(100% - 140px)">
              <div class="im-user"
                   v-for="item in Object.values(friends).reverse()"
                   :key="item.friendId"
          ...
          <div class="aside-list" v-show="type === 3">
            ...
            <div class="im-user-group"
                 v-for="item in Object.values(groups).reverse()"
                 :key="item.id"
        ...
        <groupInfo class="body-right"
                   ...
                   @updateGroupInType="updateGroupInType"
...
<script>
  ...
  import groupInfo from "./common/groupInfo.vue";
  import {reactive, getCurrentInstance, toRefs, nextTick, onBeforeUnmount} from 'vue';
  ...
  export default {
    ...
    setup() {
      ...
      const {friendData, getImFriend, removeFriend: requestRemoveFriend, getFriendRequests, changeFriendStatus} = friend();
      const {groupData, getImGroup, addGroupTopic, exitGroup: requestExitGroup, dissolveGroup: requestDissolveGroup} = group();
      ...
      let im;
      let disposed = false;
      const loadedFriendMessageIds = new Set();
      const loadingFriendMessageIds = new Set();
      const loadedGroupMessageIds = new Set();
      const loadingGroupMessageIds = new Set();

      onBeforeUnmount(() => {
        disposed = true;
        if (im?.tio?.ws) {
          im.tio.ws.onmessage = null;
        }
        im?.close();
        im = null;
      });
      ...
      function buildCssPicture() {
        ...
        if (!root || typeof webStaticResourcePrefix !== 'string' || !webStaticResourcePrefix) {
          return;
        }
        root.style.setProperty("--commentURL", `url(${JSON.stringify(webStaticResourcePrefix + "assets/commentURL.jpg")})`);
        root.style.setProperty("--imBackground", `url(${JSON.stringify(webStaticResourcePrefix + "assets/backgroundPicture.jpg")})`);
        const font = new FontFace("poetize-font", `url(${JSON.stringify(webStaticResourcePrefix + "assets/font.woff2")})`);
        font.load()
          .then(loadedFont => document.fonts.add(loadedFont))
          .catch(() => {
            // 字体加载失败时使用系统字体，不影响聊天功能。
          });
      }
      ...
      function getIm() {
        im = new Im();
        im.initWs();
        im.tio.ws.onmessage = async function (event) {
          if (disposed) {
            return;
          }
          ...
          try {
            ...
            if (!Number.isSafeInteger(message.fromId) || message.fromId <= 0) {
              return;
            }
            if (message.messageType === 1 && (!Number.isSafeInteger(message.toId) || message.toId <= 0)) {
              return;
            }
            // -1 是后端约定的公共聊天室 ID；0 与非整数仍视为非法。
            if (message.messageType === 2 && (!Number.isSafeInteger(message.groupId) || message.groupId === 0)) {
              return;
            }
            ...
            if (message.messageType === 1) {
              ...
              const isNewFriendChat = !Array.isArray(data.imMessages[message.toId]);
              if (isNewFriendChat) {
                data.imMessages[message.toId] = [];
              }
              data.imMessages[message.toId].push(message);
              if (isNewFriendChat) {
                getMessages(message.toId);
              }
              ...
              const isNewFriendChat = !Array.isArray(data.imMessages[message.fromId]);
              if (isNewFriendChat) {
                for (let i = 0; i < data.imChats.length; i++) {
            ...
            const isNewGroupChat = !Array.isArray(data.groupMessages[message.groupId]);
            if (isNewGroupChat) {
              data.groupMessages[message.groupId] = [];
            }
            data.groupMessages[message.groupId].push(message);
            if (isNewGroupChat) {
              getGroupMessages(message.groupId);
            }

            if(message.fromId === store.state.currentUser.id || isNewGroupChat) {
            ...
            // 忽略无法解析或结构不符合协议的消息，避免单条坏消息中断后续通信。
          }
        }
      }

      function sendMsg(msg, callback) {
        const success = Boolean(im?.sendMsg(msg));
        callback(success);
      }
      ...
      function isActive(e, className, type, subType, current, imType) {
        if (!$common.isEmpty(type)) {
          ...
          for (let activeClass of actives) {
            for (let tab of Array.from(document.getElementsByClassName(activeClass))) {
        ...
        for (const tab of Array.from(document.getElementsByClassName(className))) {
      ...
      function removeChatEntry(list, id) {
      ...
      function restoreSystemPanelIfChatIsEmpty() {
        if (data.subType === 2 && $common.isEmpty(data.currentChatFriendId) && $common.isEmpty(data.currentChatGroupId)) {
          data.subType = 1;
        }
      }

      function removeFriend(friendId) {
        requestRemoveFriend(friendId, removedId => {
          removeChatEntry(data.imChats, removedId);
          delete data.imMessages[removedId];
          delete data.imMessageBadge[removedId];
          loadedFriendMessageIds.delete(removedId);
          loadingFriendMessageIds.delete(removedId);
          if (data.currentChatFriendId === removedId) {
            data.currentChatFriendId = null;
          }
          restoreSystemPanelIfChatIsEmpty();
        });
      }

      function cleanGroupChat(groupId) {
        removeChatEntry(data.groupChats, groupId);
        delete data.groupMessages[groupId];
        delete data.groupMessageBadge[groupId];
        loadedGroupMessageIds.delete(groupId);
        loadingGroupMessageIds.delete(groupId);
      ...
      function exitGroup(groupId) {
        requestExitGroup(groupId, cleanGroupChat);
      }

      function dissolveGroup(groupId) {
        requestDissolveGroup(groupId, cleanGroupChat);
      }

      function updateGroupInType(groupId, inType) {
        if (groupData.groups[groupId]) {
          groupData.groups[groupId].inType = inType;
        }
      }
      ...
      function getMessages(friendId, current = 1, size = 100) {
        if (!Number.isSafeInteger(friendId) || friendId <= 0
          || loadedFriendMessageIds.has(friendId) || loadingFriendMessageIds.has(friendId)) {
          return;
        }

        loadingFriendMessageIds.add(friendId);
        $http.get($constant.baseURL + "/imChatUserMessage/listFriendMessage", {
          ...
          .then((res) => {
            if (disposed || friendData.friends[friendId] === null || friendData.friends[friendId] === undefined) {
              return;
            }
            ...
            // WebSocket 消息先于缓存落库到达且没有 id，拉取历史时必须保留这些实时消息。
            const liveMessages = Array.isArray(data.imMessages[friendId])
              ? data.imMessages[friendId].filter(message => message && message.id === undefined)
              : [];
            data.imMessages[friendId] = records.concat(liveMessages);
            loadedFriendMessageIds.add(friendId);
            nextTick(() => {
          ...
          .finally(() => {
            loadingFriendMessageIds.delete(friendId);
          });
      }

      function getGroupMessages(groupId, current = 1, size = 100) {
        // -1 是公共聊天室；0 与非整数不是有效群 ID。
        if (!Number.isSafeInteger(groupId) || groupId === 0
          || loadedGroupMessageIds.has(groupId) || loadingGroupMessageIds.has(groupId)) {
          return;
        }

        loadingGroupMessageIds.add(groupId);
        $http.get($constant.baseURL + "/imChatUserGroupMessage/listGroupMessage", {
          ...
          .then((res) => {
            if (disposed || groupData.groups[groupId] === null || groupData.groups[groupId] === undefined) {
              return;
            }
            ...
            const liveMessages = Array.isArray(data.groupMessages[groupId])
              ? data.groupMessages[groupId].filter(message => message && message.id === undefined)
              : [];
            data.groupMessages[groupId] = records.concat(liveMessages);
            loadedGroupMessageIds.add(groupId);
            nextTick(() => {
              const msgContainer = document.getElementsByClassName('msg-container');
              if (msgContainer.length > 0) {
                msgContainer[0].scrollTop = msgContainer[0].scrollHeight;
