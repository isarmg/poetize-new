          ...
          <n-tab-pane name="群设置" tab="群设置">
            <div>
              ...
              <div class="group-set"
                ...
                <div>
                  <n-switch :value="!!groups[currentGroupId].inType"
                            :loading="updatingInType"
                            :disabled="updatingInType"
                            @update:value="updateInType(currentGroupId, $event)"/>
                </div>
              </div>
...
<script>
  ...
  export default {
    emits: ['changeDataType', 'sendGroupMessage', 'exitGroup', 'dissolveGroup', 'changeAvatar', 'updateGroupInType'],
    props: {
      currentGroupId: {
        type: Number,
        required: true
    ...
    setup(props, context) {
      ...
      let data = reactive({
        ...
        updatingInType: false
      })
      let groupUserRequestId = 0;

      function updateInType(currentGroupId, inType) {
        if (data.updatingInType) {
          return;
        }
        data.updatingInType = true;
        $http.post($constant.baseURL + "/imChatGroup/updateGroup", {
          ...
          .then(() => {
            ...
            context.emit('updateGroupInType', currentGroupId, inType);
          })
          .catch((error) => {
            ElMessage({
      ...
      async function getGroupUser(groupId) {
        const requestId = ++groupUserRequestId;
        data.groupUsers = [];
        const pageSize = 100;
        const maxPages = 100;
        const groupUsers = [];
        ...
        try {
          for (let current = 1; current <= maxPages; current++) {
            const res = await $http.get($constant.baseURL + "/imChatGroupUser/getGroupUser", {
              groupId,
              current,
              size: pageSize
            });
            if (requestId !== groupUserRequestId || props.currentGroupId !== groupId) {
              return;
            }
            const records = Array.isArray(res.data?.records) ? res.data.records : [];
            groupUsers.push(...records);
            const total = Number(res.data?.total);
            if (records.length < pageSize || (Number.isFinite(total) && groupUsers.length >= total)) {
              break;
            }
          }
          if (requestId === groupUserRequestId && props.currentGroupId === groupId) {
          ...
          if (requestId === groupUserRequestId) {
            ElMessage({
              message: error.message,
              type: 'error'
            });
