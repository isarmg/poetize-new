...
<script>
  ...
  export default {
    emits: ['openFriendCircle', 'sendMsg'],
    components: {
      uploadPicture,
      emoji
    },
    ...
    setup(props, context) {
      ...
      async function requestExternalMessage(url, selectContent = value => {
        ...
        try {
          ...
          const content = selectContent(payload);
          if (typeof content !== 'string' || $common.isEmpty(content)) {
          ...
          ElMessage({
            message: error?.message || '内容获取失败，请稍后重试！',
            type: 'error'
          });
        }
      }
      ...
      function sendGuShi() {
        requestExternalMessage($constant.jinrishici, value => value?.content);
      }

      function sendYiyan() {
        requestExternalMessage($constant.yiyan);
      }

      function sendDog() {
        requestExternalMessage($constant.dog);
      }

      function sendJitang() {
        requestExternalMessage($constant.jitang);
      }

      function sendShehui() {
        requestExternalMessage($constant.shehui);
      }

      function send(e) {
        if (e && (e.ctrlKey || e.shiftKey) && e.key === 'Enter') {
          e.preventDefault();
          data.msg = data.msg + '\n';
