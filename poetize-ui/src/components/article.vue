      ...
      <div style="background: var(--background);">
        <div class="article-container my-animation-slide-bottom">
          ...
          <div class="myCenter" id="article-like" @click="subscribeLabel()">
            <el-icon class="article-like-icon" :class="{'article-like': subscribe}"><component :is="$icons.Pointer" /></el-icon>
          </div>
...
<script>
  ...
  let lineNumbersPlugin;

  function loadLineNumbersPlugin() {
    window.hljs = hljs;
    lineNumbersPlugin ||= import('highlightjs-line-numbers.js');
  ...
  export default {
    ...
    watch: {
      '$route.params.id'(newId) {
        if (newId === this.id || this.$common.isEmpty(newId)) {
          return;
        }
        this.articleRequestId += 1;
        this.newsRequestId += 1;
    ...
    methods: {
      ...
      async getNews(articleId = this.article.id) {
        const requestedSource = articleId;
        const requestId = ++this.newsRequestId;
        if (this.$common.isEmpty(requestedSource)) {
          this.treeHoleList = [];
          return;
        }
        const pageSize = 100;
        ...
        try {
          for (let current = 1; current <= 100; current++) {
            const res = await this.$http.post(this.$constant.baseURL + "/weiYan/listNews", {
              current,
              size: pageSize,
              source: requestedSource
            });
            const currentSource = this.article.id ?? this.id;
            if (requestId !== this.newsRequestId || String(currentSource) !== String(requestedSource)) {
              return;
            }
            const pageRecords = Array.isArray(res.data?.records) ? res.data.records : [];
            records.push(...pageRecords);
            const total = Number(res.data?.total);
            if (pageRecords.length < pageSize || (Number.isFinite(total) && records.length >= total)) {
              break;
            }
          }
          records.forEach(c => {
            c.content = String(c.content || "").replace(/\n{2,}/g, '<div style="height: 12px"></div>');
            c.content = c.content.replace(/\n/g, '<br/>');
            c.content = this.$common.faceReg(c.content);
            c.content = this.$common.pictureReg(c.content);
          });
          this.treeHoleList = records;
        } catch (error) {
          if (requestId === this.newsRequestId) {
            this.$message({
              message: error?.message || "最新进展加载失败！",
              type: "error"
            });
          }
        }
        ...
        this.$http.post(this.$constant.baseURL + "/article/getArticleById", {id: requestedId, password: password}, false, false)
          .then((res) => {
            ...
            if (!this.$common.isEmpty(res.data)) {
              ...
              this.getNews(this.article.id ?? requestedId);
              const md = new MarkdownIt({breaks: true}).use(multiMdTable);
              this.articleContentHtml = this.$common.sanitizeHtml(md.render(this.article.articleContent || ""));
              this.$nextTick(() => {
                if (requestId !== this.articleRequestId) {
                  return;
                }
                this.$common.imgShow(".entry-content img");
                this.highlight(requestId).catch((error) => {
                  if (requestId === this.articleRequestId) {
                    this.$message({
                      message: error?.message || "代码高亮加载失败！",
                      type: "warning"
                    });
                  }
                });
              ...
              if (!this.$common.isEmpty(password)) {
                localStorage.setItem("article_password_" + requestedId, password);
              }
              this.showPasswordDialog = false;
              if (!this.$common.isEmpty(this.$store.state.currentUser) && !this.$common.isEmpty(this.$store.state.currentUser.subscribe)) {
                const subscriptions = this.$common.parseJson(this.$store.state.currentUser.subscribe, []);
                this.subscribe = Array.isArray(subscriptions) && subscriptions.includes(this.article.labelId);
              }
            }
          })
          .catch((error) => {
            ...
            const errorMessage = typeof error?.message === "string" ? error.message : "文章加载失败！";
            if (errorMessage.startsWith("密码错误")) {
              if (!this.$common.isEmpty(password)) {
                localStorage.removeItem("article_password_" + requestedId);
                this.$message({
                  message: "密码错误，请重新输入！",
                  type: "error",
              ...
              this.tips = errorMessage.slice(4);
      ...
      async highlight(requestId) {
        ...
        $(".entry-content pre code").each(function (i, block) {
          ...
          $(block).after(
            '<button type="button" class="copy-code" aria-label="复制代码" data-clipboard-target="#hljs-' +
            i +
        ...
        this.clipboard?.destroy();
        this.clipboard = markRaw(new ClipboardJS(".copy-code"));
