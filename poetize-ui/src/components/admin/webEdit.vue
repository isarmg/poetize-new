    ...
    <div>
      ...
      <el-form :model="webInfo" :rules="rules" ref="ruleForm" label-width="100px"
        ...
        <el-form-item label="状态" prop="status">
          <el-switch @change="changeWebStatus(webInfo)" v-model="webInfo.status"></el-switch>
        </el-form-item>
        ...
        <el-form-item label="提示" prop="waifuJson">
          <div style="display: flex">
            ...
            <el-icon class="my-icon" @click="disabled = !disabled"><component :is="$icons.Edit" /></el-icon>
          </div>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
        ...
        this.$http.get(this.$constant.baseURL + "/admin/webInfo/getAdminWebInfo", {}, true)
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
              ...
              const notices = this.$common.parseJson(res.data.notices, []);
              const randomAvatar = this.$common.parseJson(res.data.randomAvatar, []);
              const randomName = this.$common.parseJson(res.data.randomName, []);
              const randomCover = this.$common.parseJson(res.data.randomCover, []);
              this.notices = Array.isArray(notices) ? notices : [];
              this.randomAvatar = Array.isArray(randomAvatar) ? randomAvatar : [];
              this.randomName = Array.isArray(randomName) ? randomName : [];
              this.randomCover = Array.isArray(randomCover) ? randomCover : [];
            }
          })
          .catch((error) => {
            this.$message({
      ...
      showNoticeInput() {
        ...
        this.$nextTick(() => {
          this.$refs.saveNoticeInput?.focus();
        });
      },
      saveNotice() {
        let param = {
      ...
      showRandomNameInput() {
        ...
        this.$nextTick(() => {
          this.$refs.saveRandomNameInput?.focus();
        });
      },
      saveRandomName() {
        let param = {
      ...
      showRandomAvatarInput() {
        ...
        this.$nextTick(() => {
          this.$refs.saveRandomAvatarInput?.focus();
        });
      },
      saveRandomAvatar() {
        let param = {
      ...
      showRandomCoverInput() {
        ...
        this.$nextTick(() => {
          this.$refs.saveRandomCoverInput?.focus();
        });
      },
      saveRandomCover() {
        let param = {
