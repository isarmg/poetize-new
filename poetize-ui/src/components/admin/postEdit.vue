  ...
  <div>
    ...
    <el-form :model="article" :rules="rules" ref="ruleForm" label-width="150px"
      ...
      <el-form-item label="内容" prop="articleContent">
        <MdEditor v-model="article.articleContent" @onUploadImg="onUploadImg"/>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      async onUploadImg(files, callback) {
        try {
          const urls = await Promise.all(Array.from(files).map(file => this.uploadArticleImage(file)));
          callback(urls);
        } catch (error) {
          this.$message({
      ...
      uploadArticleImage(file) {
        if (!matchesFileAccept(file, "image/*")) {
          return Promise.reject(new Error("仅允许上传安全的 JPG、PNG、GIF、WebP、AVIF、BMP 或 ICO 图片"));
        }
        let suffix = "";
      ...
      saveLocal(fd) {
        return this.$http.upload(this.$constant.baseURL + "/resource/upload", fd, true)
          .then((res) => {
