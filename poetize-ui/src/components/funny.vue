    ...
    <div v-for="(item, index) in funnys" :key="index">
      ...
      <div class="process-wrap">
        <el-collapse v-model="activeName" accordion @change="changeFunny(item.classify)">
          <el-collapse-item :title="item.classify.includes('音乐') ? '音乐无国界' : '笑梗不笑人'" :name="index">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async listFunny(classify) {
        const requestId = (this.funnyRequestIds[classify] || 0) + 1;
        this.funnyRequestIds[classify] = requestId;
        const records = [];
        try {
          for (let current = 1; current <= 100; current++) {
            const res = await this.$http.post(this.$constant.baseURL + "/webInfo/listResourcePath", {
              ...this.pagination,
              current,
              classify
            });
            if (this.funnyRequestIds[classify] !== requestId) {
              return;
            }
            const pageRecords = Array.isArray(res.data?.records) ? res.data.records : [];
            records.push(...pageRecords);
            const total = Number(res.data?.total);
            if (pageRecords.length < this.pagination.size || (Number.isFinite(total) && records.length >= total)) {
              break;
            }
          }
          const funny = this.funnys.find(item => item.classify === classify);
          if (funny && this.funnyRequestIds[classify] === requestId) {
            funny.data = records;
          }
        } catch (error) {
          if (this.funnyRequestIds[classify] === requestId) {
      ...
      playSound(src, playList, index) {
        ...
        try {
          ...
          requestedSource = url.href;
        } catch {
          this.$message({message: "音频地址无效！", type: "error"});
          return;
        }
        this.playList = playList;
        this.index = index;
        if (this.audio != null) {
          if (this.audio.src === requestedSource) {
            ...
            this.audio.pause();
            this.audio.src = requestedSource;
            this.audio.load();
            this.playAudio();
          }
        } else {
          this.audio = new Audio(requestedSource);
          this.playAudio();
          this.audio.onended = () => {
