      ...
      <div style="padding: 0 20px">
        ...
        <div class="card-container">
          ...
          <div v-show="card === 3" class="comment-content">
            <comment :source="love.userId || $store.state.webInfo.adminUserId || $constant.userId"
                     :type="'love'"
                     :userId="love.userId || $store.state.webInfo.adminUserId || $constant.userId"></comment>
          </div>
        </div>
      ...
      submitLove() {
        ...
        this.$http.post(this.$constant.baseURL + "/family/saveFamily", this.userLove)
          .then((res) => {
            ...
            this.userLove = createEmptyUserLove();
        ...
        this.$http.get(this.$constant.baseURL + "/family/getFamily")
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
              this.userLove = {...createEmptyUserLove(), ...res.data};
            }
          })
          .catch((error) => {
            this.$message({
        ...
        this.$http.get(this.$constant.baseURL + "/family/getAdminFamily")
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
              ...
              const adminUserId = Number(res.data.userId);
              if (Number.isSafeInteger(adminUserId) && adminUserId > 0) {
                this.weiYanPagination.userId = adminUserId;
              }
              this.getLove();
              this.countdown();
              clearInterval(this.clockTimer);
