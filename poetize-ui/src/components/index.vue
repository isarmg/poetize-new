    ...
    <loader :loading="loading">
      ...
      <template #body>
        ...
        <el-image style="animation: header-effect 2s"
                  ...
                  :src="heroCover"
                  fit="cover">
          <template #error><div class="image-slot background-image-index-error"></div></template>
        </el-image>
        <!-- 首页文字 -->
        <div class="signature-wall myCenter my-animation-hideToShow">
          ...
          <el-icon class="arrow-down-icon" @click="navigation('.page-container-wrap')"><component :is="$icons.ArrowDown" /></el-icon>
        </div>
        <!-- 首页内容 -->
...
<script>
  ...
  export default {
    ...
    computed: {
      heroCover() {
        if (!this.$common.isEmpty(this.$store.state.webInfo.backgroundImage)) {
    ...
    beforeUnmount() {
      ...
      this.guShiRequest?.abort();
      this.guShiRequest = null;
    },
    ...
    methods: {
      ...
      pushUrl(url) {
        if (!this.$common.openUrl(url)) {
          this.$message({message: "链接地址无效！", type: "error"});
        }
      },
      getGuShi() {
        let that = this;
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status >= 200 && xhr.status < 300) {
            ...
            if (result && typeof result.content === "string") {
              that.guShi = result;
              that.printerInfo = result.content;
            }
          }
        };
        xhr.send();
