...
export default {
  ...
  pushNotification(notices, isNotification) {
    ...
    const normalizedNotices = notices.filter(notice => typeof notice === "string");
    if (isNotification) {
      if (this.isEmpty(normalizedNotices)) {
        return [];
      } else {
        return normalizedNotices.filter(f => "推送标题：" !== f.substr(0, 5) &&
          "推送封面：" !== f.substr(0, 5) &&
          "推送链接：" !== f.substr(0, 5));
      }
    } else {
  ...
  pictureReg(content) {
    ...
    content = content.replace(/\[[^\[^\]]+\]/g, (word) => {
      ...
      if (index > -1) {
        ...
        return '<img loading="lazy" class="pictureReg" style="border-radius: 5px;width: 100%;max-width: 250px;display: block" src="' + url + '" title="' + title + '"/>';
      } else {
        return word;
      }
    });
  ...
  /**
   * 清洗所有最终交给 v-html 的内容，阻断事件属性、脚本协议等注入。
   */
  ...
  imgShow(select) {
    $(select).off("click.poetizePreview").on("click.poetizePreview", function () {
      ...
      $("#outerImg").off("click.poetizePreview").on("click.poetizePreview", function () {//再次点击淡出消失弹出层
        $(this).fadeOut("fast");
      });
    });
  },
